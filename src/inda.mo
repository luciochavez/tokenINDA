// Token ICRC1+ICRC2 simplificado y limpio con tokenómica de IndaToken
// Basado en una plantilla mínima funcional, con lógica añadida para:
// - Distribución inicial con periodos de bloqueo
// - Quema de tokens en transferencias (mecanismo deflacionario)
// - Emisión futura reservada (aún no implementada)
import ExperimentalCycles "mo:base/ExperimentalCycles";
import Principal "mo:base/Principal";
import HashMap "mo:base/HashMap";
import Hash "mo:base/Hash";
import Blob "mo:base/Blob";
//import { ihash } "mo:base/HashMap";
...
shared ({ caller = _owner }) actor class Token({   
    ...
    let INITIAL_ISSUANCE : Nat = 3_000_000;
    ...
    public shared ({ caller }) func mint(args : ICRC1.Mint) : async ICRC1.TransferResult {
      if (caller != _owner) {
          return #Err(#GenericError({ error_code = 100; message = "Only the owner can mint tokens" }));
      };
    ...
    //stable let holders_unlock_dates = HashMap.new<Types.Account, LockedAmount>();
    // función hash personalizada para `Types.Account`
    func accountHash(account : Types.Account) : Nat32 {
        Blob.hash(Principal.toBlob(account.owner));
    };

    // mapa estable de desbloqueos por cuenta
    var holders_unlock_dates = HashMap.HashMap<Types.Account, LockedAmount>(
        10,                           // capacidad inicial
        func(a, b) { a == b },        // función de igualdad
        accountHash                   // función de hash
    );
            ...
            for (holder in category.holders.vals()) {
                ...
                switch _mint_result {
                    ...
                    case (#Ok(_)) {
                        ...
                        ignore holders_unlock_dates.put(holder, locked_ammount);
                    };
                };
            };
        };
    ...
    func getDistributionStatus() : ?DistributionInfo {
        let releasesMap = HashMap.HashMap<Int, Nat>(
                10,
          func(a, b) { a == b },
          func(n : Int) : Nat32 { Nat32.fromNat(Int.abs(n)) } // función de hash para Int
        ); // Fecha de desbloqueo / Monto a desbloquear
        let currentTime = now();
        var amountCurrentlyBlocked = 0;
        //for (holder in Map.vals(holders_unlock_dates)) {
        for (holder in holders_unlock_dates.vals()) {
            if (currentTime < holder.unlockDate) {
                amountCurrentlyBlocked += holder.amount;
            };
            //switch (HashMap.remove<Int, Nat>(releasesMap, ihash, holder.unlockDate)) {
            switch (releasesMap.remove(holder.unlockDate)) {
                case null {
                    releasesMap.put(holder.unlockDate, holder.amount);
                };
                case (?val) {
                    releasesMap.put(holder.unlockDate, val + holder.amount);
                };
            };
        };
    ...
    func validate_transaction({ from: Principal; amount: Nat; from_subaccount: ?Blob }): { #Ok; #Err: ICRC1.TransferError } {
        ...
        //let lockedAmount = HashMap.get<Types.Account, LockedAmount>(holders_unlock_dates, ICRC1.ahash, fromAccount);
        let lockedAmount = holders_unlock_dates.get(fromAccount);
